...
log_debug() { [ "$DEBUG" = true ] && echo -e "$(date '+%Y-%m-%d %H:%M:%S') - $1"; }
...
process_barcoded_reads() {
    ...
    if [ "$DRY_RUN" = false ] && [ ! -f "${prefix}_whitelist.txt" ]; then
...
generate_statistics() {
    ...
    local outfile="${sample_dir}/${chip_name}_${lib_type}_stats.txt"
     ...
     if [ "$DRY_RUN" = false ]; then
        ...
        echo "Generate statistics file: $outfile"
    fi
}
...
run_r_analysis() {
    ...
    if [ "$DRY_RUN" = false ]; then
        ...
        echo "Rscript merge_and_analyze.R --ANXV_dir $OUTPUT_DIR/ANXV_lib --CASB_dir $OUTPUT_DIR/CASB_lib ..."
    fi
...
main() {
    ...
    if [ -f "$OUTPUT_DIR/merged/final_seurat_object.rds" ] && [ "$DRY_RUN" = false ]; then
